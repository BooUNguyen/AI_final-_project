...
class GeneticAlgorithm(QtCore.QThread):
    ...
    def __init__(self, data):
        self.averageFitness = 0     #Giá trị fitness hiện tại
        self.pastAverageFitness = 0 #Giá trị fitness trước đó dùng để so sánh với giá trị fitness vừa được tính
        self.running = True         #Trạng thái chạy của chương trình
        self.chromosomes = []       #Khởi tạo nhiễm sắc thể
        self.data = {               #Khởi tạo data gồm: Phòng học, Giảng viên, Sections, Sharing, Môn học
            'rooms': [],
            'instructors': [],
            'sections': [],
            'sharings': [],
        ...
        self.mutationRate = .10     #Tỷ lệ đột biến
        self.lowVariety = 55
        self.highestFitness = 0     #Khởi tạo max
        self.lowestFitness = 100    #Khởi tạo min
        self.elites = []
        self.matingPool = []
        self.offsprings = []
        self.tempChromosome = None  #Khởi tạo biến tạm cho nhiễm sắc thể
        self.tempSections = None    #Khởi tạo biến tạm cho Sections
        self.data = data
        self.settings = Settings.getSettings() #Lấy các giá trị thiết lập ban đầu từ file settings.json
        self.stopWhenMaxFitnessAt = self.settings['maximum_fitness'] #Biến giữ giá trị trạng thái dừng của chương trình khi đạt giá trị fitness phù hợp
        super().__init__()

    def initialization(self):
        #Tạo quần thể dựa trên số lượng cá thể tối thiểu
        self.generateChromosome(self.settings['minimum_population'])

    def generateChromosome(self, quantity): #Hàm tạo nhiễm sắc thể 
                                            #input: Số lượng nst
        for i in range(quantity):
            ...
            if not len(self.stayInRoomAssignments):
                selectedRooms = [] #Tạo mảng lưu phòng học đã chọn
